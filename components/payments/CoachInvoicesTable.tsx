...
export default function CoachInvoicesTable({ invoices }: { invoices: InvoiceRow[] }) {
  const [items, setItems] = React.useState<InvoiceRow[]>(invoices || []);
  ...
  async function setStatus(id: string, status: string) {
    ...
    if (r.ok) {
      ...
      setItems((prev) => prev.map((x) => (x.id === id ? { ...x, status: j.invoice.status } : x)));
        ...
        <div key={inv.id} className="border rounded p-3 flex items-center justify-between gap-3">
          <div className="space-y-1">
            ...
            <div className="text-sm muted">
              Student: {inv.student?.email || ""} • Plan: {inv.plan?.name || "—"}
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <a className="btn" href={"/payments/" + inv.id} target="_blank">
              Open
            </a>
          </div>
        </div>
