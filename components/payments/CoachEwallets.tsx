...
export default function CoachEwallets({ initial }: { initial: Wallet[] }) {
  ...
  async function add(e: React.FormEvent) {
    e.preventDefault();
    const r = await fetch("/api/coach/payments/ewallets", {
      method: "POST",
    ...
    if (r.ok) {
      ...
      setItems([j.wallet, ...items]);
      setProvider("");
      setHandle("");
      (window as any).dispatchEvent(
        new CustomEvent("toast", { detail: { kind: "success", msg: "Added e-wallet" } })
      );
    } else {
      (window as any).dispatchEvent(
      ...
      <ul className="grid gap-2">
        {items.map((w) => (
          <li key={w.id} className="border rounded p-3 flex items-center justify-between">
            ...
            <button className="btn" type="button" onClick={() => remove(w.id)}>
              Delete
            </button>
          </li>
        ))}
