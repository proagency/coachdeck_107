...
export default function CoachBankAccounts({ initial }: { initial: Bank[] }) {
  ...
  async function add(e: React.FormEvent) {
    e.preventDefault();
    const r = await fetch("/api/coach/payments/banks", {
      method: "POST",
    ...
    if (r.ok) {
      ...
      setItems([j.bank, ...items]);
      setBankName("");
      setAccountName("");
      setAccountNumber("");
      setBranch("");
      (window as any).dispatchEvent(
        new CustomEvent("toast", { detail: { kind: "success", msg: "Added bank" } })
      );
    } else {
      (window as any).dispatchEvent(
  ...
  async function remove(id: string) {
    ...
    if (r.ok) {
      ...
      (window as any).dispatchEvent(
        new CustomEvent("toast", { detail: { kind: "error", msg: "Failed" } })
      );
    }
  }
      ...
      <ul className="grid gap-2">
        {items.map((b) => (
          <li key={b.id} className="border rounded p-3 flex items-center justify-between">
            ...
            <button className="btn" type="button" onClick={() => remove(b.id)}>
              Delete
            </button>
          </li>
        ))}
